...
class _EditentryState extends State<Editentry> {
  ...
  Future<DateTime> _selectedDate(DateTime selectedDate) async {
  ...
  Widget build(BuildContext) {
    return Scaffold(
      ...
      body: SafeArea(child: SingleChildScrollView(
        child: Padding(padding: EdgeInsets.all(16.00),
        child: Column(
          children: <Widget>[
            TextButton(
              padding:EdgeInsets.all(0.00),
              onPressed: ()async{
                FocusScope.of(context).requestFocus(FocusNode());//dismiss the keyboard if any TextField wdget has focus.
                DateTime _pickeddate = await _selectedDate(_selectedDate);
                setState(() {
              ...
              children: <Widget>[
                TextButton(onPressed: (){
                 ...
                 child: Text('Cancel'),style: ButtonStyle(textStyle: WidgetStatePropertyAll(TextStyle(color:Colors.grey.shade100))),),
                SizedBox(width: 8.0,),
                TextButton(onPressed: (){
                  _journalEdit.action='Save';
                  String _id=widget.add ? Random().nextInt(9999).toString() : _journalEdit.journal.id;
                ...
                child: Text('Save'),
                style: ButtonStyle(textStyle: WidgetStatePropertyAll(TextStyle(color: Colors.lightGreen.shade100))),
